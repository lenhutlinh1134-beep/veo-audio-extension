// background.js — VEO Audio (AI Studio TTS only)

const Q = { queue:[], running:[], done:[], failed:[], isRunning:false, concurrency:1, mode:'text-to-speech', delayMs:3000, settings:{} };
    ...
    switch (msg.type) {
      case 'START_QUEUE':
        Q.queue = msg.prompts.map((p, i) => {
          ...
          return { id:i+1, text, name, image:null, status:'waiting', progress:0, retries:0 };
        });
        Q.concurrency = Math.min(msg.concurrency||1, 3);
        Q.mode = 'text-to-speech';
        Q.platform = 'aistudio-speech';
      ...
      case 'TEST_CONNECTION': {
        const t = await findAIStudioTab();
        if (!t) { sendResponse({ok:false,reason:'no_tab'}); break; }
        await ensureScript(t.id);
        try {
          const res = await msgTab(t.id,{type:'TEST_CONNECTION', platform:'aistudio-speech'});
...
async function processQueue() {
  ...
  while (Q.running.length < Q.concurrency && Q.queue.length > 0) {
    ...
    const tab = await findAIStudioTab();
    if (!tab) {
      move(item.id,'failed',{error:'Không tìm thấy tab AI Studio. Hãy mở aistudio.google.com/generate-speech'});
      broadcast(); continue;
    }
    await ensureScript(tab.id);
    try {
      await msgTab(tab.id, {
        type:'PROCESS_ITEM', item, mode:'text-to-speech', platform:'aistudio-speech', delayMs:Q.delayMs, settings:Q.settings
      });
    } catch(e) {
...
async function findAIStudioTab() {
...
async function ensureScript(tabId) {
  ...
  } catch(e) { console.warn('[VEO AUDIO BG] inject failed:',e.message); }
}
